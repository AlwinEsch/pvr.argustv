...
PVR_ERROR cPVRClientArgusTV::GetChannelStreamProperties(
    const kodi::addon::PVRChannel& channel, std::vector<kodi::addon::PVRStreamProperty>& properties)
{
  ...
  properties.emplace_back(PVR_STREAM_PROPERTY_INPUTSTREAM_INSTANCE_ID, "tv");
  properties.emplace_back(PVR_STREAM_PROPERTY_ISREALTIMESTREAM, "true");
  properties.emplace_back("pvr.argustv.process_class",
                          std::to_string(reinterpret_cast<uint64_t>(this)));
  properties.emplace_back("pvr.argustv.channel_unique_id", std::to_string(channel.GetUniqueId()));
  return PVR_ERROR_NO_ERROR;
}
